...
chrome.runtime.onInstalled.addListener(async () => {
  chrome.storage.local.set({
    ...
    1. Extract the title between the HTML tags <title> and </title> in the page source. If it is unavailable or too generic, make up  a page-specific title. Only if you make your own title, limit its length to 6-10 words.  The title should include specific named entities relevant to the page topic, such as technical abbreviations, software names, tool names, event names, organization names.  You may use publicly available web search to infer the title if it is not clear from the page content.
